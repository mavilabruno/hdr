...
from odoo.addons.payment_mercadopago_pe.controllers.main import MercadoPagoController
...
class AccountPaymentMethod(models.Model):
    ...
    def _get_payment_method_information(self):
        ...
        res['mercadopago_pe'] = {'mode': 'unique', 'domain': [('type', '=', 'bank')]}
        return res

class AcquirerMercadopago(models.Model):
    _inherit = 'payment.acquirer'

    def _get_mercadopago_pe_urls(self, environment):
        """ MercadoPago PerúURLS """
        if environment == 'prod':
            return {
                'mercadopago_form_url': 'https://www.mercadopago.com/mpe/checkout/pay',
                'mercadopago_rest_url': 'https://api.mercadolibre.com/oauth/token',
            }
        else:
            return {
                'mercadopago_form_url': 'https://sandbox.mercadopago.com/mpe/checkout/pay',
    ...
    def _get_providers(self, context=None):
        ...
        providers.append(['mercadopago_pe', 'MercadoPago Perú'])
        return providers

    #@api.model
    #def _get_compatible_acquirers(self, *args, currency_id=None, **kwargs):
    #    """ Override of payment to unlist MercadoPago Perúacquirers when the currency is not supported. """
    ...
    provider = fields.Selection(selection_add=[
            ('mercadopago_pe', 'MercadoPago Perú')
        ], ondelete={'mercadopago_pe': 'set default'})
    #mercadopago_client_id = fields.Char('MercadoPago PerúClient Id',required_if_provider='mercadopago')
    mercadopago_client_id = fields.Char('MercadoPago PerúClient Id', size=256)
    #mercadopago_secret_key = fields.Char('MercadoPago PerúSecret Key',required_if_provider='mercadopago')
    mercadopago_secret_key = fields.Char('MercadoPago PerúSecret Key', size=256)

    #mercadopago_email_account = fields.Char('MercadoPago PerúEmail ID', required_if_provider='mercadopago')
    mercadopago_email_account = fields.Char('MercadoPago PerúEmail ID', size=256)

    mercadopago_seller_account = fields.Char(
            'MercadoPago PerúMerchant ID',
            size=256,
            help='The Merchant ID is used to ensure communications coming from MercadoPago Perúare valid and secured.')

    mercadopago_use_ipn = fields.Boolean('Use IPN', help='MercadoPago PerúInstant Payment Notification')
    ...
    def _migrate_mercadopago_pe_account(self, context=None):
        """ COMPLETE ME """
        ...
        #for (company_id, company_mercadopago_account) in res:
            if company_mercadopago_account:
                company_mercadopago_ids = self.search([('company_id', '=', company_id), ('provider', '=', 'mercadopago_pe')], limit=1, context=context)
                if company_mercadopago_ids:
                    ...
                    mercadopago_view = self.env['ir.model.data'].get_object(cr, uid, 'payment_mercadopago_pe', 'mercadopago_acquirer_button')
                    self.create({
                        'name': 'MercadoPago',
    ...
    def mercadopago_pe_form_generate_values(self, values, tx):
...
#	            },
    ...
    def mercadopago_pe_get_form_action_url(self):
        environment = 'prod' if self.state == 'enabled' else 'test'
        return self._get_mercadopago_pe_urls(environment)['mercadopago_form_url']

    def _mercadopago_pe_s2s_get_access_token(self, ids, context=None):
        """
        ...
        for acquirer in self.browse( ids, context=context):
            ...
            tx_url = self._get_mercadopago_pe_urls( environment)['mercadopago_rest_url']
    ...
    @api.model
    def mercadopago_pe_get_reference(self, payment_id=None ):
        reference = None
        if (not payment_id):
            return reference
        mps = self.search([('provider','=','mercadopago_pe'),('mercadopago_client_id','!=',False),('mercadopago_secret_key','!=',False)])
        for mp in mps:
    ...
    def _mercadopago_pe_get_data(self, payment_id=None, reference=None ):
        data = None
...
class TxMercadoPago(models.Model):
    ...
    def _get_specific_rendering_values(self, processing_values):
        ...
        if self.provider != 'mercadopago_pe':
            return res

        return self.acquirer_id.mercadopago_pe_form_generate_values(processing_values,tx=self)
    ...
    def action_mercadopago_pe_check_status( self ):
        data = {}
        _logger.info("action_mercadopago_pe_check_status")
        for tx in self:
            ...
            try:
                data = tx.acquirer_id._mercadopago_pe_get_data(reference=acquirer_reference)
                #_logger.info(data)
                if (data):
                    tx._mercadopago_pe_form_validate(dict(data))
...
#        reference, txn_id = data.get('external_reference'), data.get('txn_id')
    ...
    def _mercadopago_pe_form_get_invalid_parameters(self, data):
        invalid_parameters = []
...
#charged_back (estado terminal) 	Se realizó un contracargo en la tarjeta de crédito.
    ...
    def _process_feedback_data(self, data):
        ...
        if (topic in ["payment"] and payment_id and self.acquirer_id):
            ...
            res = self.acquirer_id._mercadopago_pe_get_data(payment_id=payment_id)
            if (res):
                f_data.update( res )
            external_reference = f_data.get('external_reference')
        elif (external_reference):
            #DPN based on payment id, preferred...
            res = self.acquirer_id._mercadopago_pe_get_data(reference=external_reference)
            if (res):
                f_data.update( res )
        ...
        if status in ['approved', 'processed']:
            _logger.info('Validated MercadoPago Perúpayment for tx %s: set as done' % (self.reference))
            ...
            _logger.info('Received notification for MercadoPago Perúpayment %s: set as pending' % (self.reference))
            ...
            _logger.info('Received notification for MercadoPago Perúpayment %s: set as cancelled' % (self.reference))
            if (self.state not in ['cancel']):
                data.update(state='cancel', state_message=data.get('cancel_reason', ''))
            self._set_canceled()
            #self.sudo().write(data)
        else:
            error = 'Received unrecognized status for MercadoPago Perúpayment %s: %s, set as error' % (self.reference, status)
            _logger.info(error)
    ...
    def _mercadopago_pe_try_url(self, request, tries=3, context=None):
    ...
    def _mercadopago_pe_s2s_send(self, values, cc_values, context=None):
        """
         .. versionadded:: pre-v8 saas-3
         .. warning::
        ...
        headers = {
            ...
            'Authorization': 'Bearer %s' % tx.acquirer_id._mercadopago_pe_s2s_get_access_token()[tx.acquirer_id.id],
        }
        data = {
            'intent': 'sale',
            'transactions': [{
        ...
        if cc_values:
            ...
            data['payer'] = {
                'payment_method': 'mercadopago_pe',
            }
        data = json.dumps(data)

        request = urllib2.Request('https://api.sandbox.paypal.com/v1/payments/payment', data, headers)
        result = self._mercadopago_pe_try_url(request, tries=3, context=context)
        return (tx_id, result)

    def _mercadopago_pe_s2s_get_invalid_parameters(self, tx, data, context=None):
        """
         .. versionadded:: pre-v8 saas-3
         .. warning::
    ...
    def _mercadopago_pe_s2s_validate(self, tx, data, context=None):
        """
         .. versionadded:: pre-v8 saas-3
         .. warning::
        ...
        if status in ['approved']:
            ...
            _logger.info('Received notification for MercadoPago Perús2s payment %s: set as pending' % (tx.reference))
            tx.write({
                'state': 'pending',
            ...
            error = 'Received unrecognized status for MercadoPago Perús2s payment %s: %s, set as error' % (tx.reference, status)
            _logger.info(error)
            tx.write({
                'state': 'error',
                # 'state_message': error,
    ...
    def _mercadopago_pe_s2s_get_tx_status(self, tx, context=None):
        """
         .. versionadded:: pre-v8 saas-3
         .. warning::
        ...
        data = self._mercadopago_pe_try_url(request, tries=3, context=context)
